...
def build_cases(args: argparse.Namespace) -> list[EvalCase]:
    if args.protocol == "m2":
        if not args.dataset_key or not args.include_combined:
            raise RuntimeError("M2 severity requires --dataset-key and --include-combined")
...
def main() -> None:
    ...
    rows = [
        evaluate_case(model, loader, num_classes, device, case, args.protocol, args.dataset_key)
        for case in build_cases(args)
    ]
    output_dir = Path(args.output_dir)
    write_outputs(rows, output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "severity_curve_metadata.json").write_text(
        json.dumps(
            {
                "schema": "qalf-severity-curve-metadata-v1",
                "arguments": vars(args),
